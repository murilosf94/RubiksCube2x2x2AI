...
// Bloco para habilitar cores no Windows
#ifdef _WIN32
#include <windows.h>
...
// --- FUNÇÕES DE INTERFACE ADICIONADAS ---
void habilitarCores() {
#ifdef _WIN32
...
private:
    ...
    // ALTERADO: Agora retorna blocos sólidos coloridos em vez de letras
    string cor(int color) const {
        // Usamos o caractere de bloco '█' (pode ser digitado com ALT+219 no Windows)
        string block = "██"; 
        switch (color) {
...
public:
    ...
    // ALTERADO: Layout ajustado para melhor alinhamento
    ...
    // Movimentos permanecem os mesmos
    ...
    void embaralhar() {
        ...
        cout << "║ 2. Médio (15 movimentos)       ║" << endl;
        cout << "║ 3. Difícil (30 movimentos)     ║" << endl;
        cout << "╚════════════════════════════════╝" << endl;
        ...
        switch (nivel) {
            ...
            case 2: numMovimentos = 15; break;
            case 3: numMovimentos = 30; break;
...
void jogador(Cubo2x2& cubo) {
    ...
    while (movimento != -1) {
        ...
        if (movimento != -1 && movimento > 0 && movimento <=12 && !cubo.estaResolvido()) {
             cout << "\nPressione Enter para o proximo movimento...";
             cin.ignore(); cin.get();
        }
    }
}
...
int main() {
    // NOVO: Força o terminal do Windows a usar a codificação UTF-8
    #ifdef _WIN32
    ...
    switch (escolha) {
        ...
        case 3: cout << "Funcao DFS ainda nao implementada." << endl; break;
